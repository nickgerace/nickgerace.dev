...
There are many SSH clients, Linux distros, and virtualization apps, but this one will work well for beginners and advanced users alike.
...
- I will not be covering native Linux (or GNU+Linux, if that's what your prefer) desktops because those users are not the target audience of this guide. I have a major sweet spot for desktop Linux, but I also realize that these users are very resourceful and usually take on desktop Linux as a hobby.
...
What's great about using a GUI-less virtual machine is that it's **FREE** and relatively low on resource usage.
...
*What is the difference in versions?*: Canonical, the company behind Ubuntu, releases LTS versions on a two-year cadence, with the purpose of creating an exceptionally stable system with long term security updates.
Non-LTS releases benefit from a six-month cadence, which allows for greater access to new software at a negligible stability cost (at least for our purposes).
...
ISO files are disk images of optical disks, which were commonly used for CD-ROM operating systems installation.
...
You can ignore these options (if any) as we only need VirtualBox.
...
It might seem intuitive to change this folder to a backup service, such as Google Drive or Dropbox.
...
*Note*: a lot of the magic that this setup provides can be leveraged via the port forwarding tab.
...
Log in to your user and enter the following command.

```bash
sudo apt update -y && sudo apt install ssh -y && sudo systemctl enable ssh && sudo shutdown now
```

This four step command performs the following...
...
Like I had mentioned before, there are *SO* many ways at setting up an environment like this one.
However, this guide should work for a large amount of users.

Please note that the "universal" development setup is not actually universal.
